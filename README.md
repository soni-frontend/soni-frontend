# Hi there! I'm Soni 👋  

### 👩‍💻 Web Developer | Frontend Developer | React.js Developer  

I am a passionate **Web Developer** and **Frontend Developer** from Bihar, India. I enjoy building responsive, user-friendly, and modern web applications using current technologies while continuously improving my development skills through real-world projects.  

---

## 🎓 Education  

- **Bachelor of Computer Applications (BCA)**  
- **College:** Jamuni Lal College, Hajipur (Affiliated with BRA Bihar University)  
- **Status:** Final Semester Completed (Result Awaited) 🎓  

---

## 🛠 Tech Stack  

- **Languages:** HTML5, CSS3, JavaScript (ES6+)  
- **Frameworks/Libraries:** React.js, Tailwind CSS  
- **Tools & Platforms:** Git, GitHub, VS Code, Vercel  
- **Basic Knowledge:** Java, Python  
- **Core Concepts:** Data Structures, SDLC, Responsive Web Design  

---

## 🚀 Featured Projects  

### 📂 AI Spend Audit  
- **Description:** Track and analyze AI-related expenses through a responsive dashboard application.  
- **Tech Stack:** React.js, JavaScript, Tailwind CSS  
- 🔗 [View Code](https://github.com/soni-frontend/ai-spend-audit) | 🌐 [Live Demo](https://ai-spend-audit-kappa-seven.vercel.app/)  

### 📂 My Digital Card  
- **Description:** A sleek, interactive, and responsive digital business card with modern UI design.  
- **Tech Stack:** HTML5, CSS3, JavaScript  
- 🔗 [View Code](https://github.com/soni-frontend/My-digital-card) | 🌐 [Live Demo](https://soni-frontend.github.io/My-digital-card/)

### 📂 Notes App  
- **Description:** A responsive note-taking application that allows users to create, edit, and delete notes with local storage support.  
- **Tech Stack:** HTML5, CSS3, JavaScript  
- 🔗 [View Code](https://github.com/soni-frontend/Notes-App) | 🌐 [Live Demo](https://soni-frontend.github.io/Notes-App/)  

### 📂 Edustream Hub  
- **Description:** Student management dashboard for handling student records and attendance.  
- **Tech Stack:** React.js, JavaScript, Tailwind CSS  
- 🔗 [View Code](https://github.com/soni-frontend/edustream-hub) | 🌐 [Live Demo](https://edustream-hub.vercel.app/)  

### 📂 Personal Portfolio  
- **Description:** Personal portfolio website showcasing my skills, projects, and development journey.  
- **Tech Stack:** HTML5, CSS3, JavaScript  
- 🔗 [View Code](https://github.com/soni-frontend/portfolio) | 🌐 [Live Demo](https://soni-frontend.github.io/portfolio/)  

### 📂 Scientific Calculator  
- **Description:** Responsive calculator with basic and scientific calculation functions.  
- **Tech Stack:** HTML5, CSS3, JavaScript  
- 🔗 [View Code](https://github.com/soni-frontend/Scientific-Calculator) | 🌐 [Live Demo](https://soni-frontend.github.io/Calculator---App/)  

### 📂 Task Manager  
- **Description:** A task management application for organizing daily tasks efficiently.  
- **Tech Stack:** HTML5, CSS3, JavaScript  
- 🔗 [View Code](https://github.com/soni-frontend/Task-Manager) | 🌐 [Live Demo](https://soni-frontend.github.io/To-Do-List-App/)  

---

## 📫 Let's Connect  

- 💼 LinkedIn: [linkedin.com/in/soni-kumari-bca](https://www.linkedin.com/in/soni-kumari-bca)  
- 📧 Email: kumarisoni9229@gmail.com  

---

### 🚀 Always learning, building projects, and growing as a developer.  
